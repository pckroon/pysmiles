...
def test_wrong_noncycle():
    smiles = 'c1c1CC'
    with pytest.raises(ValueError):
        read_smiles(smiles, explicit_hydrogen=False)

def test_wrong_noncycle2():
    smiles = 'CC11C'
    with pytest.raises(ValueError):
        read_smiles(smiles, explicit_hydrogen=False)

def test_marker_start():
    smiles = '1CCC1'
    with pytest.raises(ValueError):
        read_smiles(smiles, explicit_hydrogen=False)

def test_wrong_aromatic():
    smiles = 'cccccc'
    with pytest.raises(ValueError):
        read_smiles(smiles, explicit_hydrogen=False)

def test_wrong_ring_bond():
    smiles = 'C=1CC-1'
    with pytest.raises(ValueError):
        read_smiles(smiles, explicit_hydrogen=False)
